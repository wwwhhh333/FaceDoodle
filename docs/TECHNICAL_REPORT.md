...
| `animation_queue` | UI → Consumer | `Anim*` dataclass (11 种) | 动画播放/关键帧/导出/纹理生成 |
...
- 6 个 Action 常量类：`Adj`(4), `Gal`(6), `Draw`(14), `Result`(5), `Disp`(6), `Anim`(16+)
...
```mermaid
flowchart TD
    P3D["3D 参考点 → 2D 投影"] --> RP["rvec / tvec 旋转矩阵"]
    RP --> PJ["四边形角点 3D 旋转 → 再投影回 2D"]
    PJ --> LP["线性插值: 正面 ↔ 侧面"]
    LP --> CL["裁剪 + 中心偏移检查"]
```
...
```mermaid
flowchart LR
    FP["帧坐标点"] -->|cv2.perspectiveTransform<br/>M_inv| CP["512×512 画布坐标"]
    CP --> SL["stamp_line()<br/>画布上绘制"]
```

- 画布大小：`CANVAS_SIZE = 512`
...
**关键设计**：绘制在规范化的 512×512 空间中进行，渲染时由 `renderer.py` 通过正向透视变换贴回脸部。
...
`KEYWORD_REGION_MAP`：**11 个面部区域**，62+ 个关键词条目：

| 区域 | 示例关键词 |
|------|-----------|
| `head_top` | 猫耳、兔耳、帽子、皇冠、光环、皇冠、角 |
| `forehead_top` | 触角、发带、头带 |
| `forehead_full` | 发饰、蝴蝶结、头巾、绑带 |
| `eyes` | 眼镜、太阳镜、眼罩、眼影、护目镜 |
| `nose` | 鼻子、猪鼻子、小丑鼻子 |
| `mouth` | 胡子、口罩、嘴唇、口红、牙齿、舌头、獠牙 |
| `cheek_left` | 腮红、面部彩绘、伤疤、雀斑、星星、爱心 |
| `cheek_right` | 同上 |
| `chin` | 下巴装饰 |
| `brows` | 眉毛 |

`REGION_ALIASES` 映射 16 个中英文别名到规范区域名。
...
| 测试文件 | 大小 | 覆盖模块 |
|---------|:----:|---------|
| `test_protocol.py` | 11.7 KB | 协议消息类型检查 |
| `test_brush.py` | 9.7 KB | 笔刷加载、图章、线段 |
| `test_animation.py` | 7.9 KB | 动画片段、引擎 |
| `test_renderer.py` | 7.8 KB | 透视变换、贴图合成 |
| `test_storage.py` | 6.9 KB | 贴纸保存/加载/删除 |
| `test_config_loader.py` | 5.8 KB | 配置加载、合并、预设 |
| `test_agent.py` | 5.2 KB | 关键词降级、region 映射 |
| `test_face_draw.py` | 5.0 KB | 画布、撤销、坐标变换 |
| `test_texture_anim.py` | 4.5 KB | 纹理动画、精灵表 |
| `test_templates.py` | 2.8 KB | 模板生成 |

**共计 278 个用例**，覆盖 AI 对话、笔刷、渲染、存储、配置、动画、协议等核心模块。
...
|------|:----:|
| `generator.py` | 无测试 |
| `comfy_manager.py` | 无测试 |
| `tracker.py` | 无测试 |
| `tracker_stickers.py` | 无测试 |
| `tracker_animation.py` | 无测试 |
| `app/ui/*` | 无测试（需烟雾测试覆盖） |
| `main.py` | 无测试 |
...
| 债务项 | 影响 | 优先级 |
|--------|------|:------:|
| `generator.py` / `tracker.py` 等核心模块无单元测试 | 修改高风险 | P0 |
| `print()` 遗留未全部迁移到 `logging` | 故障难定位 | P1 |
| 队列无大小上限 | 内存膨胀风险 | P1 |
| `cv2.imread()` 中文路径扫描未完成 | 部分路径仍可能失败 | P1 |
