...
- 动画时间轴面板可视化编辑
- ⚠️ AI 纹理动画（motion prompt → ComfyUI AnimateDiff 精灵表）仍在开发中，当前暂不可用
...
### 文字贴纸
- 点击右侧面板"添加文字"按钮，在弹出的对话框中输入文字、选择字体/字号/颜色/描边
- 文字渲染为透明 PNG 贴纸并添加到面部，支持编辑模式下的移动/旋转/缩放
- 支持 Windows 系统字体（微软雅黑、黑体、宋体、楷体等）

### 导出 GIF
- 点击右侧面板"导出GIF"按钮，录制 2 秒画面并保存为动态 GIF
- 有活跃贴纸或面部绘制内容时按钮自动启用
