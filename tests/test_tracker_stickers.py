...
class TestAddStickerInstance:
    def test_adds_to_active_stickers(self, mgr, sample_sticker_img):
        ...
        assert len(mgr.active_stickers) == 1
        assert mgr.active_stickers[0]["instance_id"] == iid
        assert mgr.active_stickers[0]["sticker_id"] == sid
        assert mgr.active_stickers[0]["location"] == "forehead_top"
    ...
    def test_respects_max_stickers(self, mgr, sample_sticker_img):
        mgr.MAX_STICKERS = 2
        mgr._add_sticker_instance(sample_sticker_img, "s1", "forehead", 1.0, "a")
        mgr._add_sticker_instance(sample_sticker_img, "s2", "forehead", 1.0, "b")
        # MAX_STICKERS is checked in _handle_add_sticker, not _add_sticker_instance.
        # _add_sticker_instance just appends — the cap enforcement is in the caller.
        # But we can still verify it's accessible.
        assert len(mgr.active_stickers) == 2
...
class TestHandleAddSticker:
    ...
    def test_empty_sticker_id_does_nothing(self, mgr):
        ...
        mgr._handle_add_sticker(msg)
        assert len(mgr.active_stickers) == 0
...
class TestHandleRemoveSticker:
    def test_removes_active_sticker(self, mgr, sample_sticker_img):
        ...
        assert len(mgr.active_stickers) == 0
        assert iid not in mgr.adjustments
...
class TestHandleLoadTemplate:
    ...
    def test_template_respects_capacity(self, mgr, sample_sticker_img):
        ...
        msg = GalLoadTemplate(template=template)
        mgr._handle_load_template(msg)
        assert len(mgr.active_stickers) == 1  # stays at 1, not 2
